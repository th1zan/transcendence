...
class Verify2FALoginView(APIView):
    ...
    def post(self, request):
        ...
        if stored_otp and stored_otp == entered_otp:
            user.is_2fa_enabled = True  # (Optional: You may want to set this already during setup)
    ...
    def post(self, request):
        ...
        if otp_code:
            ...
            if stored_otp and stored_otp == entered_otp:
                ...
                refresh = RefreshToken.for_user(user)
                response = Response({"message": "2FA successfully enabled."}, status=status.HTTP_200_OK)
                response.set_cookie("access_token", str(refresh.access_token), httponly=True, secure=False, samesite="Lax")
                response.set_cookie("refresh_token", str(refresh), httponly=True, secure=False, samesite="Lax")
                return response
            else:
            ...
            return Response({"message": "OTP sent to your email. Please verify.", "otp_required": True}, status=status.HTTP_200_OK)
