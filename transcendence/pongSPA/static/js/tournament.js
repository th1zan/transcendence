...
export function DisplayTournamentGame() {
    ...
    .then((data) => {
      ...
      document.querySelectorAll('.startGameButton').forEach(button => {
        button.addEventListener('click', event => {
          ...
          const matchID = parseInt(event.target.getAttribute('data-match-id')); // Corrected line
  ...
  document.getElementById("backToSearchButton").addEventListener("click", () => {
    validateSearch(); // Vous pouvez passer des paramètres si nécessaire
  });
}
...
export function validateSearch() {
  ...
  document.getElementById("changeTournamentButton").addEventListener("click", () => {
    const newTournamentName = document.getElementById("tournamentNameInput").value;
    if (newTournamentName) {
      localStorage.setItem("tournamentName", newTournamentName); // Mettez à jour le stockage local
